...
export default function CardSearch({ onSearch, loading = false, initialQuery = '' }) {
  ...
  // Common abilities for beginners
  const commonAbilities = [
    'Flying', 'Trample', 'First Strike', 'Deathtouch',
  ...
  const performSearch = () => {
    ...
    if (filters.abilities.length > 0) {
      const abilityQuery = filters.abilities.map(ability => `o:${ability.toLowerCase()}`).join(' OR ');
      query += query ? ` (${abilityQuery})` : `(${abilityQuery})`;
    }

    // Add rarity filter
    if (filters.rarity) {
      query += query ? ` r:${filters.rarity.toLowerCase()}` : `r:${filters.rarity.toLowerCase()}`;
    }
  ...
  const toggleFilter = (filterType, value) => {
    setFilters(prev => {
      if (filterType === 'colors' || filterType === 'types' || filterType === 'abilities') {
    ...
    <div className="card">
          ...
          {commonAbilities.map(ability => (
            <button
              ...
              style={{
                ...
                fontSize: '0.875rem',
                background: searchQuery.toLowerCase() === ability.toLowerCase() ? '#007bff' : 'white',
                color: searchQuery.toLowerCase() === ability.toLowerCase() ? 'white' : '#007bff',
        ...
        >
          ...
          {hasActiveFilters() && (
            <span style={{
              ...
              {filters.colors.length + filters.types.length + filters.abilities.length +
               (filters.rarity ? 1 : 0) + (filters.cmc ? 1 : 0)}
            </span>
          )}
        </button>
      </div>
      ...
      {showAdvanced && (
        <div style={{
          ...
          {/* Colors */}
          <div style={{ marginBottom: '1.5rem' }}>
            <h5 style={{ fontSize: '0.9rem', marginBottom: '0.75rem', color: '#495057' }}>
              Colors
            </h5>
              ...
              {colors.map(color => (
                <button
                  ...
                  style={{
                    padding: '0.5rem 0.75rem',
                    fontSize: '0.875rem',
                    background: filters.colors.includes(color.symbol) ? color.color : 'white',
                    color: filters.colors.includes(color.symbol) ?
                      (color.symbol === 'W' || color.symbol === 'G' ? '#000' : '#fff') : '#495057',
              ...
              {cardTypes.map(type => (
                <button
                  ...
                  style={{
                    padding: '0.5rem 0.75rem',
                    fontSize: '0.875rem',
                    background: filters.types.includes(type) ? '#28a745' : 'white',
              ...
              {commonAbilities.map(ability => (
                <button
                  ...
                  style={{
                    padding: '0.5rem 0.75rem',
                    fontSize: '0.875rem',
                    background: filters.abilities.includes(ability) ? '#ffc107' : 'white',
                    color: filters.abilities.includes(ability) ? '#000' : '#ffc107',
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
              {[0, 1, 2, 3, 4, 5, 6, 7, '8+'].map(cmc => (
                <button
                  key={cmc}
                  onClick={() => toggleFilter('cmc', cmc === '8+' ? '>=8' : cmc.toString())}
                  disabled={loading}
                  style={{
                    padding: '0.5rem 0.75rem',
                    fontSize: '0.875rem',
                    background: filters.cmc === (cmc === '8+' ? '>=8' : cmc.toString()) ? '#17a2b8' : 'white',
                    color: filters.cmc === (cmc === '8+' ? '>=8' : cmc.toString()) ? 'white' : '#17a2b8',
      ...
      <div style={{
        ...
        <h5 style={{ fontSize: '0.9rem', marginBottom: '0.5rem', color: '#0c5460' }}>
          💡 Search Tips
        </h5>
        <ul style={{
          fontSize: '0.8rem',
          ...
          <li>Search by name: "Lightning Bolt"</li>
          <li>Search by ability: "flying" or "trample"</li>
          <li>Use filters to narrow down results</li>
          <li>Try creature types: "dragon" or "wizard"</li>
        </ul>
      </div>
    </div>
  );
