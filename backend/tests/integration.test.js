...
describe('Integration Tests - End-to-End Workflows', () => {
  ...
  afterAll(async () => {
    ...
    for (const favouriteId of createdFavourites) {
      await request(app).delete(`/api/favorites/${favouriteId}`);
    }
  ...
  describe('User Registration and Profile Management Workflow', () => {
    test('Complete user lifecycle: create, read, update, delete', async () => {
      ...
      const userExists = listResponse.body.some(user => user.id === createdUser.id);
  ...
  describe('Card Search and Favourites Workflow', () => {
    ...
    test('Search for flying creatures and manage favourites', async () => {
      ...
      // 3. Update notes
      const updatedNotes = 'Flying creature - excellent for aerial attacks and evasion';
      const updateResponse = await request(app)
        .put(`/api/favorites/${addResponse.body.id}`)
        .send({
          notes: updatedNotes,
  ...
  describe('Message System Workflow', () => {
    test('Create and manage messages', async () => {
      ...
      const updatedText = 'UPDATED: How does the flying ability work in Magic: The Gathering?';
  ...
  describe('Performance Integration Tests', () => {
    test('Bulk operations performance', async () => {
      ...
      const testCards = favouritesResponse.body.filter(
        fav => fav.card_name.startsWith('Bulk Test Card')
      );
