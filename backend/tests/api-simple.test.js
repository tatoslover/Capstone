...
const createTestApp = () => {
  ...
  app.put("/api/favourites/:id", (req, res) => {
    ...
    if (ability_type !== undefined)
      favourites[favouriteIndex].ability_type = ability_type;
...
describe("Planeswalker's Primer API Tests (Simplified)", () => {
  ...
  describe("Card Search Integration", () => {
    ...
    test("GET /api/cards/:id should fetch specific card", async () => {
      const response = await request(app).get("/api/cards/abc123").expect(200);
  ...
  describe("Integration Workflows", () => {
    test("Complete user and favourites workflow", async () => {
      ...
      await request(app)
        .put(`/api/favourites/${favouriteResponse.body.id}`)
