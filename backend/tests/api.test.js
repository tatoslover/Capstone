...
describe('Planeswalker\'s Primer API Tests', () => {
  ...
  describe('Favourites CRUD Operations', () => {
    test('POST /api/favorites should add card to favourites', async () => {
      ...
      expect(response.body).toHaveProperty('card_name', testFavourite.card_name);
      expect(response.body).toHaveProperty('scryfall_id', testFavourite.scryfall_id);
      expect(response.body).toHaveProperty('ability_type', testFavourite.ability_type);
      expect(response.body).toHaveProperty('notes', testFavourite.notes);

      testFavouriteId = response.body.id;
    });

    test('POST /api/favorites should reject favourite without required fields', async () => {
      await request(app)
        .post('/api/favorites')
    ...
    test('PUT /api/favorites/:id should update favourite notes', async () => {
      const updatedNotes = 'Updated notes about this powerful spell';

      const response = await request(app)
        .put(`/api/favorites/${testFavouriteId}`)
        .send({
          notes: updatedNotes,
    ...
    test('PUT /api/favorites/:id should return 404 for non-existent favourite', async () => {
      await request(app)
  ...
  describe('Cleanup Operations', () => {
    test('DELETE /api/favorites/:id should remove favourite', async () => {
      const response = await request(app)
        .delete(`/api/favorites/${testFavouriteId}`)
        .expect(200);

      expect(response.body).toHaveProperty('message', 'Favorite removed');
