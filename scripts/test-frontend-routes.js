...
function checkRouteConfig() {
  ...
  log(
    `${colors.bright}${colors.blue}🔍 Frontend Route Analysis - Planeswalker's Primer${colors.reset}`,
  );
  ...
  expectedPages.forEach((page) => {
    ...
    if (checkFileExists(filePath)) {
      ...
      logTest(
        `${page.description} (${page.file})`,
        "FAIL",
        `Missing file for route: ${page.route}`,
      );
      results.failed++;
    }
  });
  ...
  if (checkFileExists(layoutFile)) {
    ...
    try {
      ...
      logTest(
        ...
        "FAIL",
        `Could not read layout file: ${error.message}`,
      );
      results.failed++;
    }
  } else {
  ...
  const favouritesExists = checkFileExists(
    path.join(pagesDir, "favourites.js"),
  );
  ...
  if (favouritesExists && favoritesExists) {
    logTest(
      ...
      "FAIL",
      "Both favourites.js and favorites.js exist - potential conflict",
    );
    results.failed++;
    ...
    logTest(
      "Spelling consistency",
      "PASS",
      "Using UK spelling (favourites.js)",
    );
    results.passed++;
    ...
    logTest(
      "Spelling consistency",
      "WARN",
      "Using US spelling (favorites.js) - should be favourites.js for consistency",
    );
    results.warnings++;
  } else {
    logTest("Spelling consistency", "FAIL", "No favourites page found");
    results.failed++;
  }
  ...
  if (checkFileExists(backendFile)) {
    ...
    try {
      ...
      const apiEndpoints = [
        {
          endpoint: "/api/users",
          pattern: /app\.(get|post|put|delete)\("\/api\/users/,
          name: "Users CRUD",
        },
        {
          endpoint: "/api/favourites",
          pattern: /app\.(get|post|put|delete)\("\/api\/favourites/,
          name: "Favourites CRUD (UK)",
        },
      ];
      ...
      results.total++;
      if (/favourites/.test(backendContent)) {
        logTest(
          "Database table spelling",
          "PASS",
          "Using UK spelling (favourites table)",
        );
        results.passed++;
      } else {
        logTest(
          "Database table spelling",
          "FAIL",
          "Could not determine table naming",
        );
        results.failed++;
      }
    } catch (error) {
      ...
      logTest(
        ...
        "FAIL",
        `Could not read backend file: ${error.message}`,
      );
      results.failed++;
    }
  } else {
    logTest(
      ...
      "FAIL",
      "server.js not found in backend/",
    );
    results.failed++;
  }
  ...
  if (results.failed > 0 || results.warnings > 0) {
    ...
    if (!favouritesExists && favoritesExists) {
      log(
        "• Rename favorites.js to favourites.js for UK spelling consistency",
        colors.yellow,
      );
    }
    ...
    if (favouritesExists && favoritesExists) {
      log(
        "• Remove duplicate page files (keep favourites.js with UK spelling)",
        colors.yellow,
      );
    }

    log(
      "• Ensure consistent spelling throughout the application",
      colors.yellow,
    );
    log(
      "• Verify backend database connection and environment variables",
      colors.yellow,
    );
  }
  ...
  log(
    "1. If favourites page 404s: Check if favourites.js exists and navigation links match",
    colors.cyan,
  );
  log(
    "2. If profile page 404s: Create profile.js page in frontend/pages/",
    colors.cyan,
  );
  log(
    "3. If backend connection fails: Check DATABASE_URL environment variable",
    colors.cyan,
  );
  log(
    "4. Test frontend routes: npm run dev (frontend) and visit http://localhost:3000",
    colors.cyan,
  );
  ...
  log(
    ...
    results.warnings > 0 ? colors.yellow : colors.green,
  );
  ...
  if (results.failed === 0 && results.warnings === 0) {
    log(
      "\n🎉 All route checks passed! Your frontend routing should work correctly.",
      colors.green,
    );
  } else if (results.failed === 0) {
    log(
      "\n⚠️  Minor issues found but no critical routing problems detected.",
      colors.yellow,
    );
  } else {
    log(
      "\n❌ Critical routing issues found. Please address the failed checks above.",
      colors.red,
    );
  }
