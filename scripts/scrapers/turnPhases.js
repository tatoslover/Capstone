...
// Scrape MTG Turn Phases information from the wiki
const scrapeTurnPhases = async () => {
  try {
    console.log('🕒 Scraping MTG Turn Phases from wiki...');

    const urls = [
      'https://mtg.fandom.com/wiki/Turn',
    ...
    // Try to scrape from turn structure page
    try {
      const { data } = await axios.get(urls[0]);
      const $ = cheerio.load(data);

      // Look for sections about turn phases
      $('h2, h3').each((i, element) => {
        const heading = $(element).find('.mw-headline').text().trim();
        if (heading.includes('phase') || heading.includes('step')) {
          let description = '';
      ...
      console.warn('Could not scrape turn phases page, using fallback');
    }

    // Comprehensive fallback data based on official MTG Comprehensive Rules
    const turnPhasesData = {
      lastUpdated: new Date().toISOString(),
      source: Object.keys(scrapedData).length > 0 ? "MTG Wiki + Comprehensive Rules" : "MTG Comprehensive Rules",
      ...
      timingRules: {
        ...
        spellSpeeds: {
          sorcerySpeed: {
            ...
            includes: ["Sorceries", "Creatures", "Planeswalkers", "Artefacts", "Enchantments", "Most activated abilities"]
          },
          instantSpeed: {
            when: "Any time you have priority",
            includes: ["Instants", "Flash creatures", "Most activated abilities", "Triggered abilities"]
          }
        },
    ...
    // Save the data
    const outputPath = path.join(__dirname, '../data/turnPhases.json');
    await fs.writeJson(outputPath, turnPhasesData, { spaces: 2 });

    console.log(`✅ Turn phases data saved to ${outputPath}`);
    console.log(`📊 Generated ${Object.keys(turnPhasesData.phases).length} phases`);
    console.log(`🕒 Included detailed step-by-step breakdown`);

    return turnPhasesData;

  } catch (error) {
    console.error('❌ Error scraping turn phases:', error.message);
    throw error;
  }
};
...
module.exports = { scrapeTurnPhases };
